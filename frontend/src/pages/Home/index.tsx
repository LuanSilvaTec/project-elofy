...
interface EnhancedTableProps {
    ...
    numSelected: number;
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof Person) => void;
...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    const createSortHandler = (property: keyof Person) => (event: React.MouseEvent<unknown>) => {
...
export default function EnhancedTable() {
    ...
    const [rows, setPerson] = React.useState<Person[]>([]);
    const [order, setOrder] = React.useState<Order>('asc');
    const [orderBy, setOrderBy] = React.useState<keyof Person>('peso');
    ...
    const handleRequestSort = (event: React.MouseEvent<unknown>, property: keyof Person) => {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelecteds = rows.map((n) => n.nome_usuario);
                            ...
                            {stableSort(filteredRow, getComparator(order, orderBy))
                                ...
                                .map((row, index) => {
                                    ...
                                    return (
                                        <TableRow
                                            hover
                                            onClick={(event) => handleClick(event, row.nome_usuario)}
                                            role="checkbox"
                                        ...
                                        >
                                            ...
                                            <TableCell align="right">{row.atleta}</TableCell>
                                        </TableRow>
                                    );
                                })}
                            {emptyRows > 0 && (
